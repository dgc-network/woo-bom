...
class WC_Bom_API {
	 ...
	 *
	 */
	public function dt_register_api_hooks() {
	 ...
	 */
	public function giar_get_posts() {
		...
		if ( 0 || false === ( $return = get_transient( 'dt_all_posts' ) ) ) {
			...
			foreach ( $all_posts as $post ) {
				$return[] = [
					...
					'upvotes'   => (int) get_post_meta( $post->ID, 'giar_upvotes', true ),
					'downvotes' => (int) get_post_meta( $post->ID, 'giar_downvotes', true ),
				];
			}
